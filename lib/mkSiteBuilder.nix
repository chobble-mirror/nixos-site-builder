    ...
    if [ $needs_rebuild -eq 1 ]; then
      ...
      if [ -f "default.nix" ]; then
        ...
        source_dir=$(nix-build \
          --no-link \ # don't pollute /nix/store
          --option trusted-public-keys "nix-community.cachix.org-1:mB9FSh9qf2dCimDSUo8Zy7bkq5CX+/rkCWyvRCYg3Fs= cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY=" \
          --option substituters "https://nix-community.cachix.org https://cache.nixos.org") || fail "nix-build failed"
      else
